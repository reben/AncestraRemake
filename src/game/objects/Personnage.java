...
public class Personnage {
	...
	public void OnJoinGame()
	{
		...
		SocketManager.GAME_SEND_SUBAREA_LIST(this);
	...
	{
		return World.getCompte(getAccID());
	}
	...
	{
		PrintWriter PW = null;
		if(get_compte().getGameThread() != null)
		{
			PW = get_compte().getGameThread().get_out();
		}
	...
	public void levelUp(boolean send,boolean addXp)
	{
		...
		if(send && _isOnline)
		{
			SocketManager.GAME_SEND_NEW_LVL_PACKET(get_compte().getGameThread().get_out(),_lvl);
			SocketManager.GAME_SEND_STATS_PACKET(this);
			SocketManager.GAME_SEND_SPELL_LIST(this);
		}
	}
	...
	public void addXp(long winxp)
	{
		...
		if(_isOnline)
		{
			if(exLevel < _lvl)SocketManager.GAME_SEND_NEW_LVL_PACKET(get_compte().getGameThread().get_out(),_lvl);
			SocketManager.GAME_SEND_STATS_PACKET(this);
		}
	}
	...
	{
		_curCarte.addPlayer(this);
		if(get_compte().getGameThread() != null && get_compte().getGameThread().get_out() != null)
		{
			SocketManager.GAME_SEND_STATS_PACKET(this);
	...
	*/
	...
	{
		String packet = "";
		for (Objet obj : get_compte().getBank().getBankItems().values())
		{
			packet += "O" + obj.parseItem() + ";";
		}
		if (get_compte().GetBankKamas() != 0)
	...
	{	
		for (Objet entry : get_compte().get_bank().values()) 
		{
			if(entry.getTemplate().getType() == 85)
				continue;
			if (entry.getTemplate().getID() == exObj.getTemplate().getID() && entry.getStats().isSameStats(exObj.getStats()))
				return entry;
		}
		return null;
	}
	...
	public void removeFromBank(int guid, int qua) 
	{
		...
		if(get_compte().get_bank().get(guid) == null)
		{
	...
	public String parseToFriendList(int guid)
	{
		...
		str.append("?;");//FIXME
		str.append(_name).append(";");
		if(get_compte().isFriendWith(acc.get_pseudo()))
		{
